...
import {
  Camera, Thermometer, Wind, CloudSnow, Mountain, RefreshCw, ChevronRight, ChevronLeft,
  Star, AlertTriangle, MapPin, Info, Menu, X, ExternalLink, TrendingUp, Activity,
  Snowflake, ArrowUp, ArrowDown, CheckCircle, XCircle, MinusCircle, Coffee, Heart,
  Loader, WifiOff
} from 'lucide-react';
...
const Card = ({ children, className = '', onClick }) => (
  <div
    onClick={onClick}
    className={`bg-slate-800/50 backdrop-blur rounded-xl border border-slate-700/50 overflow-hidden ${onClick ? 'cursor-pointer hover:bg-slate-800/70' : ''} ${className}`}
  >
    {children}
  </div>
);
...
const Update = ({ u }) => {
  const colors = {
    ...
    snow: 'bg-cyan-500/20 text-cyan-400',
    ops: 'bg-amber-500/20 text-amber-400',
    weather: 'bg-blue-500/20 text-blue-400',
    road: 'bg-emerald-500/20 text-emerald-400'
  };
  return (
    <div className="flex gap-3 p-3 bg-slate-800/30 rounded-lg">
      <div className={`flex-shrink-0 px-2 py-1 rounded text-xs font-medium ${colors[u.type] || 'bg-slate-500/20 text-slate-400'}`}>
        {(u.type || 'info').toUpperCase()}
      </div>
      <div className="flex-1">
        <p className="text-sm text-slate-200">{u.msg || u.message}</p>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {secondary.map(n => (
                  ...
                  >
                    <n.icon className="w-5 h-5 text-cyan-400" /><span>{n.label}</span>
                  </button>
                ))}
              </div>
            </div>
...
const Home = ({ setPage }) => {
  ...
  const WEATHER = data?.WEATHER || [],
    FORECAST = data?.FORECAST || { hourly: [], daily: [] },
    SNOW = data?.SNOW,
    LIFTS = data?.LIFTS || [],
    RUNS = data?.RUNS || [],
    AVAL = data?.AVAL;
  ...
  const openLifts = LIFTS.filter(l => l.status === 'open').length,
    groomedRuns = RUNS.filter(r => r.groomed).length;

  const hasSnow = !!SNOW,
    hasLifts = LIFTS.length > 0,
    hasRuns = RUNS.length > 0,
    hasTemps = WEATHER.length > 0,
    hasForecast = FORECAST.daily.length > 0,
    hasAval = AVAL?.level != null;
...
/**
 * ✅ UPDATED CAMS PAGE:
 * - supports backend fields: image OR src
 * - supports pass report cards: type === 'pass_report' with passReportId
 * - when you click into the pass-report card, it shows the WSDOT pass report info inline
 */
const Cams = () => {
  const { data } = useApp();
  const CAMS = data?.CAMS || [];
  const passReports = data?.ROADS?.passReports || {};
  const [filter, setFilter] = useState('all');
  const [sel, setSel] = useState(null);
  const [imgErr, setImgErr] = useState({});

  // Normalize cam fields so we can render thumbnails
  const normalizeCam = (c) => ({
    ...c,
    // allow either src OR image from backend
    src: c.src || c.image || null,
    name: c.name || c.title || 'Camera',
    desc: c.desc || c.description || '',
    category: c.category || 'other',
    type: c.type || (c.src || c.image ? 'image' : 'external')
  });

  const validCams = CAMS.map(normalizeCam).filter(c => c.src || c.link);
  ...
  if (sel) {
    ...
    const isPassReport = cam.type === 'pass_report' && cam.passReportId;
    const report = isPassReport ? passReports?.[cam.passReportId] : null;
      ...
      <div className="space-y-4">
        ...
        <Card>
          <div className="aspect-video bg-slate-900 flex items-center justify-center relative overflow-hidden">
            {(cam.type === 'image' || cam.src) && cam.src && !imgErr[cam.id] ? (
              <img
                src={cam.src}
                alt={cam.name}
              ...
              <div className="text-center px-4">
                ...
                {cam.link && (
                  <a
                    ...
                    className="block mt-4 text-cyan-400 text-sm"
                  >
                    Open → 
                  </a>
                )}
            ...
            {isPassReport && (
              <div className="mt-4 p-4 bg-slate-900/40 rounded-lg border border-slate-700/50">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-xs text-slate-500 uppercase font-semibold">Pass Report</p>
                    <p className="text-sm text-slate-300 mt-1">
                      Updated: <span className="text-white">{report?.lastUpdated || report?.fetchedAt || '—'}</span>
                    </p>
                  </div>
                  {report?.source && (
                    <a
                ...
                {report?.error ? (
                  <p className="text-sm text-rose-400 mt-3">{report.error}</p>
                ) : (
                  <div className="mt-3 space-y-2 text-sm text-slate-300">
                    <div><span className="text-slate-500">Temperature:</span> <span className="text-white">{report?.temperature || '—'}</span></div>
                    <div><span className="text-slate-500">Elevation:</span> <span className="text-white">{report?.elevation || '—'}</span></div>
                    <div><span className="text-slate-500">Travel eastbound:</span> <span className="text-white">{report?.travelEastbound || '—'}</span></div>
                    <div><span className="text-slate-500">Travel westbound:</span> <span className="text-white">{report?.travelWestbound || '—'}</span></div>
                    <div><span className="text-slate-500">Conditions:</span> <span className="text-white">{report?.conditions || '—'}</span></div>
                    <div><span className="text-slate-500">Weather:</span> <span className="text-white">{report?.weather || '—'}</span></div>
                  </div>
                )}
              </div>
            )}
        ...
        <div className="flex gap-2">
          ...
          {categories.map(cat => (
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm capitalize ${filter === cat
                ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30'
                : 'bg-slate-800 text-slate-400'
                }`}
            >
          ...
          <Card key={c.id} onClick={() => setSel(c.id)}>
            <div className="aspect-video bg-slate-900 flex items-center justify-center overflow-hidden">
              {c.src && !imgErr[c.id] ? (
                ...
                <span className="text-4xl">{c.icon || (c.type === 'pass_report' ? '🛣️' : '📷')}</span>
              )}
            </div>
            <div className="p-3">
              ...
              <div key={i} className="flex items-center justify-between py-2 border-b border-slate-700/50 last:border-0">
                ...
                <div className="text-right">
                  <p className="text-sm font-semibold text-white">
                    {fmt.temp(d.hi, settings.units)}{d.lo != null && ` / ${fmt.temp(d.lo, settings.units)}`}
                  </p>
...
const Lifts = () => {
  ...
  const [view, setView] = useState('status'), [filter, setFilter] = useState('all');
  if (LIFTS.length === 0) return <NotAvailable message="Lift status not available" />;
  const open = LIFTS.filter(l => l.status === 'open').length, hold = LIFTS.filter(l => l.status === 'hold').length;
  const list = filter === 'all' ? LIFTS : LIFTS.filter(l => l.status === filter);
  if (view === 'lightboard') return (
    <div className="space-y-4">
...
const Runs = () => {
  ...
  const [filter, setFilter] = useState('all'), [groomed, setGroomed] = useState(false);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
        {[{ id: 'all', l: 'All' }, { id: 'favorites', l: '⭐' }, { id: 'green', l: '●' }, { id: 'blue', l: '■' }, { id: 'black', l: '◆' }, { id: 'double-black', l: '◆◆' }].map(f => (
          <button
            ...
            className={`flex-shrink-0 px-3 py-1.5 rounded-lg text-sm ${filter === f.id ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30' : 'bg-slate-800 text-slate-400'}`}
          >
            {f.l}
          </button>
        ))}
      ...
      <button
        ...
        className={`w-full flex items-center justify-between p-3 rounded-lg ${groomed ? 'bg-cyan-500/20 border border-cyan-500/30' : 'bg-slate-800'}`}
      >
      ...
      {passes.map((p, i) => (
        <Card key={p.id || i} className="p-4">
          <div className="flex items-start justify-between mb-2">
            ...
            <Badge status={p.status === 'advisory' ? 'hold' : p.status?.includes('open') ? 'open' : 'closed'} />
          </div>
  ...
  <Card className="p-4">
    ...
    <div className="space-y-4">
      ...
      <div className="border-t border-slate-700 pt-4">
        ...
        {[{ l: 'Crystal Mountain', u: 'https://www.crystalmountainresort.com' }, { l: 'NWAC', u: 'https://nwac.us' }, { l: 'WSDOT Passes', u: 'https://wsdot.com/travel/real-time/mountainpasses' }].map(x => (
...
const About = () => (
  <div className="space-y-4">
    ...
    <Card className="p-4">
      ...
      <p className="text-sm text-slate-300">NWS • WSDOT • NWAC</p>
    </Card>
  </div>
);
...
const Privacy = () => (
  <Card className="p-4">
    ...
    <div className="space-y-4 text-sm text-slate-300">
      ...
      <p><strong className="text-white">Data:</strong> Fetched from NWS, WSDOT, NWAC</p>
    </div>
  </Card>
);
...
export default function App() {
  ...
  useEffect(() => {
    try { localStorage.setItem('cpSettings', JSON.stringify(settings)); } catch { }
  }, [settings]);
